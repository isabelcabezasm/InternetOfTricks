...
void loop()
{
	...
	if (color != 0)
	{
		...
		switch (color) {
		case 1:
			client.request(web::http::methods::POST, builder.to_string(), L"{Color:'rojo'}", L"application/json");
			break;
		case 2:
			client.request(web::http::methods::POST, builder.to_string(), L"{Color:'verde'}", L"application/json");
			break;
		case 3:
			client.request(web::http::methods::POST, builder.to_string(), L"{Color:'azul'}", L"application/json");
			break;
		case 4:
			client.request(web::http::methods::POST, builder.to_string(), L"{Color:'amarillo'}", L"application/json");
			break;
		}
		color = 0;
	}
